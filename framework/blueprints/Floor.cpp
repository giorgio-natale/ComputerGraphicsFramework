...
namespace fmwk {
    ...
    void Floor::buildEntity() {
        ...
        floorVertices.push_back({{0,0,0}, {0, _height}, fmwk::Y , {fmwk::X, 1.0f}}); //00
        floorVertices.push_back({{_width,0,0}, {_width, _height}, fmwk::Y , {fmwk::X, 1.0f}}); //10
        floorVertices.push_back({{_width,0,-_height}, {_width, 0},fmwk::Y , {fmwk::X, 1.0f}}); //11
        floorVertices.push_back({{0,0,-_height}, {0, 0},fmwk::Y , {fmwk::X, 1.0f}}); //11
        ...
        indices.push_back(3);
        indices.push_back(0);
        gameEngine->addModel<fmwk::VertexWithNormalAndTan>("FloorModel", VERTEX_WITH_NORMAL_AND_TANGENT, floorVertices, indices, glm::vec3(0), glm::quat(1,0,0,0), glm::vec3(1));
        ...
        floorEntity->addComponent(std::make_unique<fmwk::GGXMaterial>(gameEngine->getBoundTextureByName("rockNormal").getTexture(), gameEngine->getBoundTextureByName("rockRoughness").getTexture()));
