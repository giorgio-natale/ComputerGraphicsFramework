...
class Assignment07 : public BaseProject {
	...
	void localInit() {
        ...
        //setup scene
        fmwk::GameEngine* gameEngine = fmwk::GameEngine::getInstance();
        auto characterEntity =
                std::make_unique<fmwk::Entity>(
                        "Character",
                        glm::vec3(-41.50, 0.0, -19.5),
                        fmwk::EulerVector(0,0,0)
                        );

        auto cameraEntity = std::make_unique<fmwk::Entity>("Camera");

        auto cameraComponent = std::make_unique<fmwk::PerspectiveCamera>(
                "MainCamera",
                0.1f,
                100.0f,
                glm::radians(45.0f)
        ...
        auto characterController = std::make_unique<fmwk::CharacterController>(
                "CharacterController",
                cameraEntity->getTransform(),
                2.0f
                );

        auto cameraController = std::make_unique<fmwk::CameraController>(
                "CameraController",
                characterEntity->getTransform(),
                glm::radians(120.0f),
                1.5f,
                0.25f
                );
        characterEntity->addComponent(std::move(characterController));
        cameraEntity->addComponent(std::move(cameraController));
